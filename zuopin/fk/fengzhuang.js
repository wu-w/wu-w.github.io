...
// 轮播图
	...
	// tuZ 				第一个轮播图的初始优先级
	// lisColor 		轮播点原始的颜色
function zlunbo(leftan,rightan,pic,bigbannerBox,lis,colorArr,tuactiveZ,liactivebgcolor,LunBoTime,tuZ,lisColor,shu){
	// $(function(){
	const tup=$(pic);
	const banner=$(bigbannerBox)[0];
	const li=$(lis);
	const leftaj=$(leftan)[0];
	const rightaj=$(rightan)[0];
	...
	//轮播函数
		function move(type="l"){
			...
			if(type=="l"){
				//左移动
				next++;
				if(next>tup.length-1){
					next = 0;	
				}
				tup[next].style.left=imgW + "px";
				animate(tup[now],{left:-imgW},500);
			}else if(type=="r"){
				//右移动
				next--;
				if(next<0){
			...
			animate(tup[next],{left:0},500,function(){
				//轮播结束
					flag=true
	...
	// }
	leftaj.onmouseover=function(){
		clearInterval(t);
	}
	rightaj.onmouseover=function(){
		clearInterval(t);
	}
	leftaj.onclick=function(){
		t=setInterval(move,LunBoTime)
	}
	rightaj.onclick=function(){
		t=setInterval(move,LunBoTime)
	}
}
	...
	function jiedian(pic,Bigbox,lis,Leftbnt,Rightbnt,num){
		...
		function  downla(items,itemul) {
			...
			for(let i=0;i<ul.length;i++){
				let h=parseInt(window.getComputedStyle(ul[i],null).height);//获取高
				console.log(h)
