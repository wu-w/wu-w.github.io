...
// 轮播图
	...
	// tuZ 				第一个轮播图的初始优先级
	// lisColor 		轮播点原始的颜色
function zlunbo(pic,bigbannerBox,lis,colorArr,tuactiveZ,liactivebgcolor,LunBoTime,tuZ,lisColor,shu,leftanjiu,rightanjiu){
	// $(function(){
	const tup=$(pic);
	const banner=$(bigbannerBox)[0];
	const li=$(lis);
	const color=colorArr;
	const leftanj=$(leftanjiu)[0];
	const rightanj=$(rightanjiu)[0];
	...
	function move(){
		// if(type=="l"){
            ...
            for(let i=0;i<tup.length;i++){
                tup[i].style.zIndex=tuactiveZ;
                li[i].style.background=liactivebgcolor;
            }
            tup[num].style.zIndex=tuZ;
            li[num].style.background=lisColor;
            banner.style.background=color[num];
		// }


	}
    leftanj.onmouseover=function(){
        clearInterval(t);
    }
    // rightanj.onmouseover=function(){
    //     clearInterval(t);
    // }
    leftanj.onclick=function(){
        // if(flag){
        move("l")
        // }
    }
    // rightanj.onclick=function(){
    //     // if(flag){
    //     	move("r")
    //     // }
    // }
			...
			window.onscroll=function(){
				for(let i=0;i<section.length;i++){
					if(obj.scrollTop+0.5*ch>=section[i].offsetTop){
						...
						list[i].style.background=bgmnexty;
					}
					// if(obj.scrollTop+ch>=section[i].offsetTop){
